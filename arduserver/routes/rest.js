...
exports.entry = (req, res, next) => {
    ...
    DBSensor.getSensorData({mac: MAC}, (err, data) => {
        if (err) {
            console.log('getSensorData Err: ' + err)
            return next(err)
        }
        if (data !== undefined) return res.status(200).send('already have') // запись про текущий день и час уже есть
        DBSensor.setNewData({mac: MAC, name: arr[2], value: arr[3]}, function (err, id) {
            if (err) {
...
exports.entry2 = (req, res, next) => {
    ...
    DBSensor.getSensorData2({mac: arr[1]}, (err, data) => {
        if (err) {
            console.log('getSensorData2 Err: ' + err)
            return next(err)
        }
        if (data !== undefined) return res.status(200).send('already have') // запись про текущий день и час уже есть
        DBSensor.setNewData2({mac: arr[1], name: arr[2], boxValue: arr[3], roomValue: arr[4]}, (err, id) => {
            if (err) {
                console.log('setNewData2 Err: ' + err)
                return next(err)
            }
